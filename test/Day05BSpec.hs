...
--    it "can jump to a location if a value is non-zero" $ do
--        let code = [5,5,4,99,104,42,99] -- output 42 if 42 /= 0 
--            mock = MockLineIO { setInput = "", getOutput = "" }
--            st   = execState (run code) mock
--        getOutput st `shouldBe` "42"
